...
- Background scheduler generating JSONL output for active tasks — all active tasks run concurrently per tick
- Graceful shutdown on `SIGINT`/`SIGTERM` — drains the current scheduler tick and in-flight HTTP requests before exiting
...
| `-nominatim-delay` | `1s` | Minimum delay between Nominatim requests — keep at `1s` for the public instance; set to `0` (or e.g. `50ms`) for a self-hosted instance |
...
Results are cached by rounded coordinates (~1 km grid resolution) so that nearby points within the same area reuse the same lookup. A serialising lock held across the entire HTTP call enforces at most 1 in-flight Nominatim request at any time. The `-nominatim-delay` flag (default `1s`) sets the minimum gap between consecutive calls — required by the public Nominatim usage policy. Set it to `0` or a small value (e.g. `-nominatim-delay=50ms`) when running a self-hosted instance. Concurrent tasks that miss the cache for the same coordinates are served by the first caller's result without issuing a duplicate request.
...
Every `-scheduler-interval`, the scheduler finds all `active` tasks and generates `count` RTB requests for each concurrently (one goroutine per task), writing them as newline-delimited JSON to:

```
{out-dir}/task_{correlation_id}_{unix_timestamp}.jsonl
```

After all tasks in a tick have completed, tasks are grouped by their effective SFTP target and one zip archive is created per group:
