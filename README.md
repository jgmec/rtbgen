...
Results are cached by rounded coordinates (~1 km grid resolution) so that nearby points within the same area reuse the same lookup. A serialising lock held across the entire HTTP call enforces at most 1 in-flight Nominatim request at any time, with at least 1 second between consecutive calls, complying with the Nominatim usage policy. Concurrent tasks that miss the cache for the same coordinates will be served by the first caller's result without issuing a duplicate request.
